...
export async function openAIResponsesAdapter({imageBytes,derivedAsset,env}){
  ...
  const body={model,store:false,truncation:'disabled',input:[{role:'user',content:[{type:'input_text',text:instruction},{type:'input_image',image_url:`data:${derivedAsset.mime};base64,${bytesToB64(imageBytes)}`,detail:'high'}]}],text:{format:{type:'json_schema',name:'record_scoresheet_observations',strict:true,schema:OBS_SCHEMA}}};
