...
test('signed remote observations require and verify a trusted gateway key in Capsule export/verification',async()=>{
  ...
  let env=JSON.parse(fs.readFileSync('fixtures/recognition/clean-fixture-001/envelope.json','utf8'));env.gameId=g.id;env.pageId=p.id;env.sourceSha256=p.sha256;env.requestId='capsule-signed-request1';env.derivedAsset={...env.derivedAsset,id:asset.id,sha256:asset.sha256,mime:asset.mime,width:asset.width,height:asset.height,preprocessorVersion:asset.preprocessorVersion};env.gateway={build:'record-gateway/0.3.1',keyId:'test-gateway-key1',signedAt:'2026-09-09T00:00:00.000Z'};
