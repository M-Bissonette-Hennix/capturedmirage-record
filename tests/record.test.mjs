import test from 'node:test';import assert from 'node:assert/strict';import {webcrypto} from 'node:crypto';if(!globalThis.crypto)globalThis.crypto=webcrypto;import {appendMove,buildCertification,createGame,invalidateCertification,localPgnDate,replaceMoveAt,validateRecord,verifyCertification} from '../src/record.js';
...
test('new games use local calendar date rather than UTC slicing',()=>{const fake={getFullYear:()=>2026,getMonth:()=>8,getDate:()=>25};assert.equal(localPgnDate(fake),'2026.09.25');});
